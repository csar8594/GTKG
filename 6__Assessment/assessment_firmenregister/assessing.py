...
def main():
    ...
    randIndexSub = getRandomHotels(hotelArr)
    hotelArrSub = getSubHotelArr(hotelArr, randIndexSub)

    instanceCompleteness = getInstanceCompleteness(hotelArrSub)
    populationCompleteness = getPopulationCompletenes(hotelArr)
    synValidity = synValid(hotelArrSub)
    semValidity = semValid(hotelArrSub)

    print(str(instanceCompleteness) + " out of " + str(len(hotelArrSub)) + " hotel(s) have all 4 mandatory properties")
